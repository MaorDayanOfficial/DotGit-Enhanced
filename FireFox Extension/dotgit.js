...
function notification(title, message) {
    if (title === "Download status") {
        if (!notification_download) return;
    } else {
        if (!notification_new_git) return;
    }
...
function startDownload(baseUrl, downloadFinished) {
    ...
    const downloadedIndex = new Map(); // path → index in downloadedFiles for O(1) lookup
    ...
    function decompressObject(hash) {
        ...
        const fileIdx = downloadedIndex.get(objPath);
        if (fileIdx === undefined) return null;
        const file = downloadedFiles[fileIdx];
        ...
        if (running_tasks === 0 && waiting === 0) {
            ...
            if (typeof URL.createObjectURL === 'function') {
                zip.generateAsync({type: "blob"}).then(function (zipBlob) {
                    ...
                    debugLog("ZIP generation failed:", err);
                    notification("Download status", "Failed to create ZIP: " + err.message);
                    downloadFinished(false, downloadStats);
                });
            } else {
    ...
    function downloadFile(path, decompress, callback) {
        if (walkedPaths.has(path)) { downloadZip(); return; }
        ...
        if (running_tasks >= max_connections) {
            ...
            fetch(baseUrl + GIT_PATH + path, {
                ...
                if (typeof buffer !== "undefined") {
                    ...
                    downloadedIndex.set(path, downloadedFiles.length - 1);
